...
Model model;
Data data(model);
int nq, nv, na;  // 位置变量数、速度变量数、执行器数量
...
void initRobotModel(const std::string& urdf_path) {
    ...
    // 获取模型维度
    nq = model.nq;   // 位置变量数 (浮动基座 + 关节角度)
    nv = model.nv;   // 速度变量数
    na = nv - 6;     // 执行器数量 (减去浮动基座的6个自由度)
    
    std::cout << "Robot model loaded: " << urdf_path << "\n";
    std::cout << "Total DOFs: " << nv << " (Actuated: " << na << ")\n";
}
...
public:
    ...
    TrajectoryVariables(const std::string& name) 
        : VariableSet((nq + nv) * N, name)  // 每个时间点有位置和速度
    {
        // 初始化轨迹为0
    ...
    // 获取特定时间点的状态
    VectorBlock getState(int k) const {
        return values_.segment(k * (nq + nv), nq + nv);
    }
    
    // 设置状态
    void setState(int k, const VectorXd& state) {
        values_.segment(k * (nq + nv), nq + nv) = state;
    }
    
    // 获取位置
    VectorBlock getPosition(int k) const {
        return values_.segment(k * (nq + nv), nq);
    }
    
    // 获取速度
    VectorBlock getVelocity(int k) const {
        return values_.segment(k * (nq + nv) + nq, nv);
    }
    ...
    VecBound GetBounds() const override {
        ...
        for (int k = 0; k < N; k++) {
            ...
            for (int i = 0; i < nq; i++) {
                // 浮动基座位置有较大范围，关节角度限制
                if (i < 3) {  // 位置 (x,y,z)
                    bounds[k*(nq+nv)+i] = Bounds(-10.0, 10.0);
                } else if (i < 7) {  // 四元数方向
                    // 四元数归一化由约束处理
                    bounds[k*(nq+nv)+i] = NoBound;
                } else {  // 关节角度
                    bounds[k*(nq+nv)+i] = Bounds(
...
public:
    ...
    DynamicsConstraint(const std::string& name)
        : ConstraintSet(nv * (N-1), name)  // 每个时间点有nv个约束
    {}
    ...
    VecBound GetBounds() const override {
        ...
        for (int k = 0; k < N-1; k++) {
            // 前6个自由度 (浮动基座) 必须为0
            for (int i = 0; i < 6; i++) {
                bounds[k*nv+i] = Bounds(0.0, 0.0);
            }
            
            // 关节力矩可以有边界 (这里设为无约束)
            for (int i = 6; i < nv; i++) {
                bounds[k*nv+i] = NoBound;
            }
        }
...
public:
    ...
    double GetCost() const override {
        ...
        for (int k = 1; k < N-1; k++) {
            ...
            VectorXd tau_k = rnea(model, data, q_k, v_k, a_k).tail(na);
            
            // 上一个时间点的力矩
            VectorXd q_km1 = var->getPosition(k-1);
            VectorXd v_km1 = var->getVelocity(k-1);
            VectorXd a_km1 = (v_k - v_km1) / dt;
            VectorXd tau_km1 = rnea(model, data, q_km1, v_km1, a_km1).tail(na);
...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < N; k++) {
        VectorXd state = x_opt.segment(k*(nq+nv), nq+nv);
        VectorXd q = state.head(nq);
        VectorXd v = state.tail(nv);
